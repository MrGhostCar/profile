...
public class StudentIntegrationTests extends MockMvcTestConfig {
    ...
    StudentRequestDTO correctStudent = new StudentRequestDTO(null, "Kovacs Endre", "kovacs.endre@gmail.com");
    ...
    public void whenPostSuccessful_thenReturnCreated() throws Exception {
        mockMvc.perform(post("/student").contentType(APPLICATION_JSON)
                .content(getJson(correctStudent))).andDo(print()).andExpect(status().isCreated());
    }

    @Test
    public void givenEmailInvalid_whenPostMade_thenReturnBadRequest() throws Exception {
        mockMvc.perform(post("/student").contentType(APPLICATION_JSON)
                .content(getJson(invalidEmailStudent))).andDo(print()).andExpect(status().isBadRequest());
    }
